...
BATCH_RETRIES        = 2
BATCH_HARD_TIMEOUT   = 45  # seconds — kills a batch if yf.download() itself hangs
                            # (curl_cffi doesn't always honor timeout= internally)
WINDOW_START         = "14:15"
WINDOW_END           = "15:30"
...
def fetch_intraday(symbols: list, scan_date: date, trade_date: date) -> dict:
    ...
    for i in range(0, len(yf_syms), YF_BATCH_SIZE):
        ...
        for attempt in range(BATCH_RETRIES + 1):
            # NOTE: intentionally NOT using ThreadPoolExecutor as a context
            # manager here — its __exit__ calls shutdown(wait=True), which
            # would block until the hung download finishes anyway, defeating
            # the timeout below. We let the executor (and its stuck thread,
            # if any) leak in the rare hang case rather than block forever;
            # daemon-like leaked threads are an acceptable tradeoff versus
            # freezing the whole scan cycle.
            ...
            try:
                ...
                try:
                    raw = future.result(timeout=BATCH_HARD_TIMEOUT)
                    ex.shutdown(wait=False)
                except FutureTimeoutError:
                    # yf.download() itself hung (curl_cffi doesn't always
                    # honor timeout= internally). Stop waiting and move on —
                    # otherwise the whole scan cycle stalls here forever.
                    log(f"[WARN] Batch {batch_num} attempt {attempt + 1} "
                        f"HUNG past {BATCH_HARD_TIMEOUT}s hard timeout, "
